...
$erk = \app\models\Bemor::find()->all();
    ...
    foreach ($erk as $x) {
        $tash = $x->tashxis;
        array_push($tashxis_arr, $tash);
    };
    $tashllar = array_unique($tashxis_arr);
    // Faqat sonlarni sanab turish
    // Faqat sonlarni olish
    $arruyMalumotlar =$tashxis_arr; 
    $uniqMalumotlarCount = array_count_values($arruyMalumotlar);
    $uniqMalumotlar = array_values($uniqMalumotlarCount);
    // Natijani chiqarish
    echo "<script>";
    echo "var jsTashxis = " . json_encode($tashllar) . ";";
    echo "</script>";
    echo "<script>";
    echo "var jsTashxis_soni = " . json_encode($uniqMalumotlar) . ";";
    echo "</script>";
?>
...
<script>
    ...
    window.addEventListener("load", (event) =>{
           ...
           for (var key in jsTashxis) {
            ...
            for (var r = 0, n = []; r < a;) {
                var i = Math.floor(Math.random() * (t.max - t.min + 1)) + t.min,
                    o = Math.floor(61 * Math.random()) + 15;
                n.push([e, i, o]), e += 864e5, r++
            }
            return n
            }
